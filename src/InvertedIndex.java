...
public class InvertedIndex {
	...
	public static HashMap<String, TreeMap<String, Integer>> createInMemoryIndex(
			WikilinksParser reader)  throws IOException {		
		int pages_counter = 1;
		...
		while (reader.hasMoreItems()) {
			...
			WikiLinkItem i = reader.nextItem();		  
			...
			for (Mention m : i.mentions) {
				...
				wiki_freebase_map_ids.put(key, m.freebase_id);
	...
	public static void outputTermDocidPairs(
		...
		while (reader.hasMoreItems()) {
			...
			HashSet<String> seen_anchors = new HashSet<String>();
			for (Mention m : i.mentions) {
				if (seen_anchors.contains(m.wiki_url+";" + m.freebase_id + ";" + m.anchor_text)) continue;
				seen_anchors.add(m.wiki_url+";" + m.freebase_id + ";" + m.anchor_text);	
				
				String key_wiki = m.wiki_url.substring(m.wiki_url.lastIndexOf('/'));
				String key_freebase = m.freebase_id;
				char out_fop = 'a';
